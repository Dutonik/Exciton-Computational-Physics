...
/*
THIS VERSION OF THE CODE USES TILES OF 10nm x 10nm WHEREAS THE ABOVE BLOCK IS FOR 20nm x 20nm TILES. THIS BLOCK IS MORE PRECISE BUT SLOWER.
#include <sys/stat.h>
#include <cstdio>
#include <iostream>
#include <cmath>
...
for (int i = 0; i < active_Excitons.size(); i++) {
    ...
    //MAIN LOOP (2249 cycles = 1 nanosecond)
    int j=0;
    while (j<250){
        //Get the total # of living excitons
        int exciton_population_size = active_Excitons.size();
        ...
        //convert variance from units of tiles (:= 100nm^2) to square micrometers
        converted_variance_average = variance_average*0.0001;
        ...
        //increase while-loop index
        j++;
        //increase time by unit (1 cycle = 4.4464e-4 nanoseconds)
        simulation_time += 0.00044464;
    }
    varianceFile.close();
    cout<<"Simulated "<<simulation_time<<" nanoseconds of diffusion \n";
}
...
double bivariate_gaussian(double x, double y) {
    ...
    const double x0 = (Nx-1)/2.0; // center x -- default to halfway along x grid
    const double y0 = (Ny-1)/2.0; // center y -- default to halfway along x grid (this centers the laser impulse at the center of the grid)
    const double sigma_x = 42.4; // standard deviation along x-axis
    const double sigma_y = 42.4; // standard deviation along y-axis
    double exponent = -((x - x0) * (x - x0) / (2 * sigma_x * sigma_x) + (y - y0) * (y - y0) / (2 * sigma_y * sigma_y));
